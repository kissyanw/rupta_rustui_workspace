...
type Modifier = class<{
    ...
    #[late] let ref clone_ty_late_ref: Option<String> = Some(self.f().to_string());
...
fn test_copy_ty() {
    let m = Modifier::new();
    assert_eq!(m.get().copy_ty().unwrap(), 0_usize);
    assert_eq!(m.raw().copy_ty().as_ref().unwrap(), &0_usize);
}
...
fn test_class_ty() {
    let m = Modifier::new();
    assert_eq!(m.get().class_ty().unwrap().get().foo(), 0_usize);
    assert_eq!(m.raw().class_ty().as_ref().unwrap().get().foo(), 0_usize);
}
...
fn test_copy_ty_mut() {
    let m = Modifier::new();
    assert_eq!(m.get().copy_ty_mut().unwrap(), 0_usize);
    assert_eq!(m.raw().copy_ty_mut().get().unwrap(), 0_usize);
...
fn test_class_ty_mut() {
    let m = Modifier::new();
    assert_eq!(m.get().class_ty_mut().unwrap().get().foo(), 0_usize);
    assert_eq!(m.raw().class_ty_mut().get_cloned().unwrap().get().foo(), 0_usize);
...
fn test_copy_ty_ref() {
    let m = Modifier::new();
    assert_eq!(m.get().copy_ty_ref().as_ref().unwrap(), &0_usize);
    assert_eq!(m.raw().copy_ty_ref().as_ref().unwrap(), &0_usize);
}
...
fn test_clone_ty_ref() {
    let m = Modifier::new();
    assert_eq!(m.get().clone_ty_ref().as_ref().unwrap(), "0");
    assert_eq!(m.raw().clone_ty_ref().as_ref().unwrap(), "0");
}
...
fn test_class_ty_ref() {
    let m = Modifier::new();
    assert_eq!(m.get().class_ty_ref().as_ref().unwrap().get().foo(), 0_usize);
    assert_eq!(m.raw().class_ty_ref().as_ref().unwrap().get().foo(), 0_usize);
}
...
fn test_copy_ty_ref_mut() {
    let m = Modifier::new();
    assert_eq!(m.get().copy_ty_ref_mut().unwrap(), 0_usize);
    assert_eq!(m.raw().copy_ty_ref_mut().borrow().unwrap(), 0_usize);
...
fn test_clone_ty_ref_mut() {
    let m = Modifier::new();
    assert_eq!(m.get().clone_ty_ref_mut().as_ref().unwrap(), "0");
    assert_eq!(m.raw().clone_ty_ref_mut().borrow().as_ref().unwrap(), "0");
...
fn test_class_ty_ref_mut() {
    let m = Modifier::new();
    assert_eq!(m.get().class_ty_ref_mut().as_ref().unwrap().get().foo(), 0_usize);
    assert_eq!(m.raw().class_ty_ref_mut().borrow().as_ref().unwrap().get().foo(), 0_usize);
...
fn test_class_ty_late_mut() {
    let m = Modifier::new();
    assert_eq!(m.get().class_ty_late_mut().unwrap().get().foo(), 0_usize);
    assert_eq!(m.raw().class_ty_late_mut().get().unwrap().as_ref().unwrap().get().foo(), 0_usize);
